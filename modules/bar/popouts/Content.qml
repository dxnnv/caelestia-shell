...
Item {
    ...
    Item {
        ...
        Repeater {
            model: ScriptModel {
                values: SystemTray.items.values.filter(i => i.hasMenu && i.status !== Status.Passive && !GlobalConfig.bar.tray.hiddenIcons.includes(i.id) && !Services.TrayFilter.shouldHide(i))
            }
